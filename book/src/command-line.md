...
`:insert-stream-output` (or `::`) uses the same shell argument rules, but inserts the command and then streams stdout/stderr into the buffer as output arrives. If the primary selection is non-empty when the command is given, the selection is appended to the shell command as the final argument. For linewise selections, one trailing line ending is removed before appending.

Only one command streams at a time. Invoking `:insert-stream-output` while a stream is running queues the new command behind it instead of starting a second one, and queued commands run in order into the same buffer, each headed by its own command line. The statusline spinner shows how many commands are still waiting. Streamed commands are run without stdin, so a command that would prompt for input reads EOF rather than blocking the queue. A command that fails doesn't stop the queue: the next command still runs, so pass a single `cmd1 && cmd2` when the chain should stop on failure. `:cancel-stream` (or `:::`) is a full stop: it kills the running command and drops everything queued behind it.
