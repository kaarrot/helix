...
- `Space-m c` reads selected git log lines, with the bottom line as the base
  (not included): one line is `COMMIT` versus the working tree; two or more
  lines are `OLDER..NEWER`.
