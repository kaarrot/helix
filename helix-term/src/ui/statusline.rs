    ...
    F: Fn(&mut RenderContext<'a>, Span<'a>) + Copy,
{
    ...
    // Check if there's a global diff range set
    if let Some(ref diff_range) = context.editor.diff_range {
        let content = format!(" [DIFF: {}] ", diff_range.display());
        let style = context.editor.theme.get("ui.statusline.mode.diff");
        write(context, Span::styled(content, style));
        return;
    }
...
{
    let title = {
        // For virtual documents (merge/diff views), show the display name
        // For regular files, show the actual absolute path
        let path = if context.doc.display_name_override.is_some() {
            context.doc.display_name()
        } else {
            context.doc.path()
                .as_ref()
